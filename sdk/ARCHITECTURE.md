...
- Developers building integrations or host applications using `@sctg/cline-core`
...
```mermaid
flowchart LR
  shared["@sctg/cline-shared"]
  llms["@sctg/cline-llms"]
  agents["@sctg/cline-agents"]
  core["@sctg/cline-core"]
  apps["Host Apps"]
...
- hub discovery, the detached hub daemon, and the `@sctg/cline-core/hub/daemon-entry` subpath
- host-side hub client adapters (`NodeHubClient`, `HubSessionClient`, `HubUIClient`, `connectToHub`) exported from `@sctg/cline-core/hub`
...
1. Host constructs a `RuntimeHost` through `@sctg/cline-core`.
2. `@sctg/cline-core` selects `LocalRuntimeHost` through `packages/core/src/runtime/host.ts`.
...
4. `@sctg/cline-core` prepares a local bootstrap artifact from `localRuntime`, then builds the runtime from it.
5. `@sctg/cline-core` creates an `Agent` from `@sctg/cline-agents`.
6. `@sctg/cline-agents` runs the loop using `@sctg/cline-llms` handlers.
7. `@sctg/cline-core` persists state, artifacts, and metadata.
...
1. Host constructs a `RuntimeHost` through `@sctg/cline-core`.
2. `@sctg/cline-core` selects `HubRuntimeHost` or `RemoteRuntimeHost` through `packages/core/src/runtime/host.ts`.
3. When no compatible local hub is already discovered, `@sctg/cline-core` can spawn a detached hub daemon and reconnect through discovery.
...
5. The hub-hosted runtime executes the agent loop using `@sctg/cline-agents` and `@sctg/cline-llms`.
6. `@sctg/cline-core` hub services broker sessions, events, approvals, schedules, and client-owned runtime capabilities such as session-local tool executors.
...
8. Hub client adapters exported from `@sctg/cline-core/hub` (`NodeHubClient`, `HubSessionClient`, `HubUIClient`, `connectToHub`) translate command/reply and event streams into host-facing APIs.
...
2. `@sctg/cline-shared/remote-config` caches the bundle when configured.
...
5. `@sctg/cline-core` exposes the app-facing integration wrapper that applies extensions, telemetry, and session metadata to `StartSessionInput`.
6. `@sctg/cline-core` consumes the prepared local overrides during local bootstrap.
...
Cross-package logging uses a small injected interface exported from `@sctg/cline-shared`:
...
- **`TelemetryLoggerSink` (`@sctg/cline-core`)** — an **`ITelemetryAdapter`** that mirrors telemetry events and metrics into a `BasicLogger`. It is a telemetry sink, not a host logging implementation.
...
- `@sctg/cline-agents` owns the generic turn-preparation seam:
...
- `@sctg/cline-core` owns compaction policy:
...
Do not move these concerns into `@sctg/cline-agents`:
...
Do not make `@sctg/cline-core` organization- or provider-specific.

If a capability is truly generic and app-facing, add a generic core seam. Reusable remote-config parsing, materialization, and upload primitives belong in `@sctg/cline-shared/remote-config`.
...
`@sctg/cline-core` ships a file-based automation subsystem under
...
1. **Spec parser** (`cron/specs/cron-spec-parser.ts`): parses YAML frontmatter + body
   ...
   Types live in `@sctg/cline-shared` under `src/cron/cron-spec-types.ts`
...
- `@sctg/cline-shared` — shared types, contracts, and low-level utilities
- `@sctg/cline-llms` — provider integrations and model manifests
- `@sctg/cline-agents` — the agent loop and tool orchestration
- `@sctg/cline-core` — the main SDK with session management, hub, and configuration
