...
```bash
cline plugin install https://github.com/TEA-ching/cline/blob/main/sdk/examples/plugins/weather-metrics.ts
...
```bash
cline plugin install https://github.com/TEA-ching/cline/blob/main/sdk/examples/plugins/typescript-lsp/index.ts
...
```bash
npm add @sctg/cline-core
```

Add `@sctg/cline-agents` or `@sctg/cline-llms` only if you intentionally want lower-level control. For RPC client helpers, prefer importing from `@sctg/cline-core` when you want the app-facing SDK surface.

Current SDK layering:

- `@sctg/cline-core` owns config discovery/watchers, runtime plugin loading, and the context pipeline
- context compaction is core-owned and runs through turn preparation before model calls
- most app integrations should stay on the `@sctg/cline-core` surface unless they intentionally need lower-level agent or model control
