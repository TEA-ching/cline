...
- `@sctg/cline-shared`: shared contracts, schemas, path helpers, hook engine, extension registry, low-level utilities
- `@sctg/cline-llms`: provider settings/config, model catalogs, provider manifests, gateway contracts, handler creation
- `@sctg/cline-agents`: stateless agent loop, tool orchestration, hook/extension runtime, event streaming
- `@sctg/cline-core`: stateful orchestration, session lifecycle, storage, config watching, plugin loading, default tools, telemetry. Exposes `@sctg/cline-core/hub` for discovery, the detached daemon entry, WebSocket clients, and session/UI client adapters, plus `@sctg/cline-core/hub/daemon-entry` for launching the shared daemon

### Dependency Direction

```mermaid
flowchart TD
  shared["@sctg/cline-shared"] --> llms["@sctg/cline-llms"] & agents["@sctg/cline-agents"] & core["@sctg/cline-core"]
  llms --> agents & core
  agents --> core
...
- model/provider schemas or handler behavior: `@sctg/cline-llms`
- stateless loop, tool orchestration, streaming, hook/extension runtime: `@sctg/cline-agents`
- session lifecycle, storage, config watching, default tools, plugin loading, telemetry, hub runtime services, hub discovery, hub daemon spawn, and session-oriented client helpers (`HubSessionClient`, `HubUIClient`, `connectToHub`): `@sctg/cline-core` (hub pieces live under `src/hub/`)
- remote-config schemas, managed instruction materialization, blob upload metadata, and OpenTelemetry config normalization: `@sctg/cline-shared/src/remote-config`
...
```sh
bun -F @sctg/cline-shared test
bun -F @sctg/cline-llms test
bun -F @sctg/cline-agents test
bun -F @sctg/cline-core test:unit
bun -F @sctg/cline-cli test:unit
```
...
- For `@sctg/cline-llms` provider/model routing rules, follow [packages/llms/AGENTS.md](./packages/llms/AGENTS.md).
