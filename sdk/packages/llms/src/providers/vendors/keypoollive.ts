...
function selectNextKey(
	...
	const eligible = keys.filter(
		(k) => k.type !== "expired" && !isQuotaExhausted(k),
	);
	...
	const sorted = [...usable].sort((a, b) => {
		const sa: KeyStats24h = statsMap.get(keyMask(a.key)) ?? {
			completionTokens: 0,
			promptTokens: 0,
			requestCount: 0,
		};
		const sb: KeyStats24h = statsMap.get(keyMask(b.key)) ?? {
			completionTokens: 0,
			promptTokens: 0,
			requestCount: 0,
		};
		if (sa.completionTokens !== sb.completionTokens)
			return sa.completionTokens - sb.completionTokens;
		if (sa.promptTokens !== sb.promptTokens)
			return sa.promptTokens - sb.promptTokens;
...
function getEffectiveRemoteConfig(): KeypoolRemoteStorageConfig | null {
	...
	if (remoteStorageConfig) {
		return isHttpUrl
			? { ...remoteStorageConfig, workerUrl: envUrl }
			: remoteStorageConfig;
	}
...
function mergeWithSessionStats(
	...
	for (const [keyHint, sessionStat] of session) {
		...
		if (existing) {
			merged.set(keyHint, {
				completionTokens:
					existing.completionTokens + sessionStat.completionTokens,
...
export const createKeypoolliveProvider: GatewayProviderFactory = (config) => ({
	...
	async *stream(
		...
		const stateKey = opts.scope
			...
			: providerName;
		const vaultCacheKey = isAuto
			? (opts.scope ?? getRequiredVaultUrl())
			: undefined;
		...
		const statsMap = isAuto
			? opts.loadUsageStats24h
				? mergeWithSessionStats(
						stateKey,
						await opts.loadUsageStats24h(providerName),
					)
				: await readProviderUsageStats24hMerged(
						providerName,
						stateKey,
						opts.remoteStorage,
					)
		...
		for (let attempt = 0; attempt < MAX_KEY_ATTEMPTS; attempt++) {
			...
			if (!isAuto) {
				...
				const resolved = resolveNextApiConfig(
					vault,
					providerName,
					modelId,
					statsMap,
			...
			if (resolvedVaultModel) {
				const caps = new Set<GatewayModelCapability>(
					context.model.capabilities ?? [],
				);
				(resolvedVaultModel.supportsImages ?? false)
					? caps.add("images")
					: caps.delete("images");
				(resolvedVaultModel.supportsPromptCache ?? false)
					? caps.add("prompt-cache")
					: caps.delete("prompt-cache");
				(resolvedVaultModel.supportsTools ?? false)
					? caps.add("tools")
					: caps.delete("tools");
