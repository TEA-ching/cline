import { createGateway, type GatewayProviderSettings } from "@sctg/cline-llms";
import type {
...
// `@sctg/cline-shared` (see `packages/shared/dist/identifier.ts`), but
 ...
 * `@sctg/cline-core`, which constructs models itself to share gateway/telemetry
 ...
 * runtime builds an `AgentModel` internally via `@sctg/cline-llms`. This is the
 ...
 * construct the model itself via `@sctg/cline-llms`.
 */
...
export class AgentRuntime {
	...
	private async addUserReminderMessage(text: string): Promise<AgentMessage> {
		...
		// Tags this synthetic system-injected message so checkpoint run
 ...
 *
 * while `@sctg/cline-core` (which owns model construction) continues to use
