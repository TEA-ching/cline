...
export const WebFetchRequestSchema = z.object({
	...
	formats: z
		.array(WebFetchFormatSchema)
		.optional()
		.describe(
			"Output formats to request. Defaults to [{type:'markdown'}]. Supports markdown, html, rawHtml, links, screenshot, summary, json (with schema/prompt), question (with question), highlights (with query).",
		),
	onlyMainContent: z
		.boolean()
		.optional()
		.describe(
			"Strip navigation, ads, and footers. Defaults to true. Set false to include full page content.",
		),
	onlyCleanContent: z
		.boolean()
		.optional()
		.describe(
			"LLM-based boilerplate removal pass (beta). Can be combined with onlyMainContent.",
		),
	waitFor: z
		.number()
		.int()
		...
		.optional()
		.describe(
			"Milliseconds to wait for JavaScript-rendered content before scraping. Use for SPAs and lazy-loaded pages.",
		),
	mobile: z
		.boolean()
		.optional()
	...
	proxy: z
		...
		.optional()
		.describe(
			"Anti-bot proxy level: basic (fast), enhanced (stronger, costs 5 credits), auto (retry with enhanced on failure).",
		),
	headers: z
		.record(z.string(), z.string())
		.optional()
		.describe(
			'Custom HTTP headers to send with the request, e.g. {"Authorization":"Bearer token"}.',
		),
	includeTags: z
		.array(z.string())
		.optional()
	...
	excludeTags: z
		...
		.optional()
		.describe(
			'HTML tags to exclude from the output, e.g. ["nav","footer","aside"].',
		),
	location: z
		.object({
			country: z
				.string()
				.describe("ISO 3166-1 alpha-2 country code, e.g. 'US', 'DE', 'JP'."),
			languages: z
				.array(z.string())
				.optional()
				.describe("Preferred languages in order, e.g. ['en-US']."),
		})
		.optional()
		.describe("Geographic context for locale-specific content."),
	actions: z
		.array(WebFetchActionSchema)
		.optional()
		.describe(
			"Browser automation steps to run before scraping: click, write, press, wait, scroll, screenshot, executeJavascript.",
		),
});
 ...
 */
export type WriteMarkdownToDocxInput = z.infer<
	typeof WriteMarkdownToDocxInputSchema
>;
