...
export function createWriteMarkdownToDocxExecutor(
   ...
   * ```
   */
  onDocxBuffer: (listener: (data: { buffer: Uint8Array, markdown: string }) => void) => () => void;
   ...
   * ```
   */
  onProgress: (listener: (data: { progress: number, status: string }) => void) => () => void;} 
  {
	// Event emitters for cross-environment communication
	const docxBufferEmitter = new EventEmitter<{ buffer: Uint8Array, markdown: string }>();
	const progressEmitter = new EventEmitter<{ progress: number, status: string }>();
	...
	const executor: WriteMarkdownToDocxExecutor = async (
		...
		try {
			...
			progressEmitter.emit({ progress: 0.1, status: "Starting markdown to DOCX conversion" });

			if (isBrowser) {
				// Browser environment: return buffer via events
				progressEmitter.emit({ progress: 0.3, status: "Converting markdown to DOCX document" });

				const document = await markdownDocx(markdown);
				progressEmitter.emit({ progress: 0.7, status: "Generating DOCX buffer" });
				...
				progressEmitter.emit({ progress: 1.0, status: "Conversion completed successfully" });
				...
				progressEmitter.emit({ progress: 0.4, status: "Preparing output directory" });

				const outputDir = dirname(outputPath);
				await mkdir(outputDir, { recursive: true });

				progressEmitter.emit({ progress: 0.6, status: "Converting markdown to DOCX document" });

				const document = await markdownDocx(markdown);
				progressEmitter.emit({ progress: 0.8, status: "Writing DOCX file to disk" });

				const docxBuffer = await Packer.toBuffer(document);
				writeFileSync(outputPath, docxBuffer);

				progressEmitter.emit({ progress: 1.0, status: "File saved successfully" });
				return `Successfully converted markdown to DOCX and saved to ${outputPath}`;
			} else {
				throw new Error("Unsupported execution environment: neither browser nor Node.js detected");
			}
		} catch (error) {
