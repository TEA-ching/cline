...
function formatFirecrawlResponse(
	...
	if (data.actions?.screenshots && data.actions.screenshots.length > 0) {
		...
		sections.push(
			data.actions.screenshots.map((s, i) => `[${i + 1}] ${s}`).join("\n"),
		);
		sections.push("");
	}
	...
	if (
		...
		data.actions.javascriptReturns.length > 0
	) {
...
export async function fetchWithFirecrawl(
	...
	try {
		...
		if (options?.onlyCleanContent !== undefined)
			requestBody.onlyCleanContent = options.onlyCleanContent;
