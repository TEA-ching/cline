...
export function createWebFetchExecutor(
	...
	if (crawlerResolver) {
		return createSmartWebFetchExecutor(
			crawlerResolver,
			nativeExecutor,
			timeoutMs,
		);
...
function createNativeWebFetchExecutor(
	options: Omit<WebFetchExecutorOptions, "crawlerResolver">,
): WebFetchExecutor {
	const {
		timeoutMs = 30000,
		maxResponseBytes = 5_000_000,
