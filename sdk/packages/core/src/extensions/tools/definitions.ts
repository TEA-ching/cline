...
export function createWebSearchTool(
	...
	return createTool<SearchWebInput, ToolOperationResult[]>({
		...
		execute: async (input, context) => {
			...
			const query =
				typeof validatedInput === "string"
					? validatedInput
					: validatedInput.query;
			const allowedDomains =
				typeof validatedInput === "object"
					? validatedInput.allowed_domains
					: undefined;
			const blockedDomains =
				typeof validatedInput === "object"
					? validatedInput.blocked_domains
					: undefined;
 ...
 * @example
 * ```typescript
 * import { Agent, createDefaultTools } from "@sctg/cline-core"
...
export function createDefaultTools(
	...
	if (enableWriteMarkdownToDocx && executors.writeMarkdownToDocx) {
		tools.push(
			createWriteMarkdownToDocxTool(executors.writeMarkdownToDocx, config),
		);
	}
