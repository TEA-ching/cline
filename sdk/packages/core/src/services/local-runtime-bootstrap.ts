import { resolveProviderRequestHeaders } from "@sctg/cline-llms";
import type {
	AgentConfig,
	AgentEvent,
	AgentHooks,
...
import { hasRuntimeConfigExtension } from "@sctg/cline-shared";
...
import { createKeypoolCrawlerResolver } from "@sctg/cline-llms";
import type { KeypoolEventHandler } from "@sctg/cline-shared";
import {
	createWebFetchExecutor,
	createWebSearchExecutor,
} from "../extensions/tools/executors";
...
function augmentWithKeypoolWebFetch(
	...
	if (!result.webFetch) {
		console.log(
			"[Bootstrap] KEYPOOL_VAULT_URL found, injecting SmartWebFetch executor",
		);
		result.webFetch = createWebFetchExecutor({ crawlerResolver: resolver });
	}
	if (!result.webSearch) {
		console.log(
			"[Bootstrap] KEYPOOL_VAULT_URL found, injecting WebSearch executor",
		);
...
export function buildKeyPoolLiveHeader(input: {
	...
	if (!existingUa) {
		headers["User-Agent"] =
			`Cline/${process.env.npm_package_version || "1.0.0"}`;
	}
	input.onEvent?.({
		type: "user-agent-set",
	...
	const resolvedHeaders =
		config.providerId === "keypoollive"
			...
			: resolveProviderRequestHeaders({
					...
					openAiCodex: {
						accountId:
							sessionProviderConfig?.accountId ?? stored?.auth?.accountId,
						accessToken:
...
export async function prepareLocalRuntimeBootstrap(
	...
	const effectiveToolExecutors = augmentWithKeypoolWebFetch(
