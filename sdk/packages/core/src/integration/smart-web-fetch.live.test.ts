...
} from "../extensions/tools/executors/index";
...
describe("Smart Web Fetch Integration Tests", () => {
			...
			resolve: async () => {
				if (process.env.FIRECRAWL_API_KEY) {
					return {
						...
						protocol: "firecrawl",
						endpoint:
							process.env.FIRECRAWL_ENDPOINT ?? "https://api.firecrawl.dev",
				...
				if (process.env.EXA_API_KEY) {
					return {
						crawlerName: "env-exa",
						protocol: "exa",
						endpoint: process.env.EXA_ENDPOINT ?? "https://api.exa.ai",
	...
	run(HAS_FC || HAS_EXA)(
		...
		async () => {
			...
			const smartExecutor = createSmartWebFetchExecutor(
				makeEnvResolver(),
				nativeExecutor,
			);
			const result = await smartExecutor(
				"https://example.com",
				"summarize",
				{} as any,
			);
