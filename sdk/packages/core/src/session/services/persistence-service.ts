...
export class UnifiedSessionPersistenceService {
	...
	// See .backport-agent/customizations.yaml id "checkpoint-runcount-fix". Not
	// exposed on the public RuntimeHost/ClineCore API - internal use only (via
	// LocalRuntimeHost.persistSessionMetadata), so it can't be a breaking change
	// for external callers of updateSession() who expect replace-semantics.
	//
	...
	// while blindly overwriting whatever the other committed in between -
	// a classic lost update, silently invisible to both callers (both see
