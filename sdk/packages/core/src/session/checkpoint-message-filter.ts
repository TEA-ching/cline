...
// See .backport-agent/customizations.yaml id "checkpoint-runcount-fix" for the
// full rationale and the exact removal criteria. Short version: upstream's
// checkpoint runCount used to count every low-level agent run/continue
// invocation instead of genuine user-authored turns, causing checkpoint
// restore to fail almost universally once a provider issues internal
// continuations (retries, key rotation, etc.) between visible turns. This
// file provides a single, shared definition of "a genuine user turn" for the
// persisted conversation format (`LlmsProviders.Message`), reused by both
// checkpoint creation (via checkpoint-run-counting.ts) and checkpoint restore
// so the two can never drift from each other again.
 ...
 * `role: "user"` message across the codebase (see customizations.yaml for
 * the full list of call sites tagged as part of this fix).
 */
