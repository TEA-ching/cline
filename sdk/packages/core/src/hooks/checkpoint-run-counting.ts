...
// full rationale and the exact removal criteria. This file is the live-runtime
// counterpart of ../session/checkpoint-message-filter.ts: it defines "a
// genuine user turn" for the `AgentMessage` shape used by the agent runtime's
// snapshot (`sdk/packages/agents/src/agent-runtime.ts`), as opposed to the
// persisted `LlmsProviders.Message` shape used for stored conversation
// history. The two shapes model tool results differently (a distinct `role:
// "tool"` here, vs. `tool_result` content blocks folded into `role: "user"`
// there), so they need separate filters - do not try to unify them.
import type { AgentMessage } from "@sctg/cline-shared";
