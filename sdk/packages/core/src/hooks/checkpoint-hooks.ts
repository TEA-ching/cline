...
import type { AgentHooks, BasicLogger } from "@sctg/cline-shared";
...
type CreateCheckpointHooksOptions = {
	 ...
	 * avoids clobbering concurrent metadata writes - see checkpoint history
	 * loss bug where a stale `readSessionMetadata()` snapshot was merged
	 * against and then written back, dropping entries written in between.
	 */
...
export function createCheckpointHooks(
	...
	return {
		// ── FORK BUGFIX (keypool-live): checkpoint-runcount-fix ─────────────
		// See .backport-agent/customizations.yaml id "checkpoint-runcount-fix"
		// for the full rationale and the exact removal criteria. There used
		// to be a `beforeRun` hook here incrementing a mutable `runCount` on
		// every root-level run/continue invocation - but upstream's
		// `AgentRuntime.run()`/`.continue()` both funnel into the same
		// `execute()`, which calls `beforeRun` unconditionally regardless of
		// whether this is a new user turn or an internal continuation (retry,
		// provider key rotation, etc). That made `runCount` count internal
		// invocations instead of user-visible turns, silently diverging from
		// the webview's own turn counter (apps/vscode/src/sdk/sdk-checkpoints.ts)
		// and causing checkpoint restore to fail ("No checkpoint found at or
		// before run N") almost universally. `beforeRun` is no longer needed:
		// `beforeModel` below now derives the run number directly from the
		// count of genuine user-authored messages in the snapshot, which is
