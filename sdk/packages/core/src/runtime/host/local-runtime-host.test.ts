...
import type { MessageWithMetadata } from "@sctg/cline-llms";
import {
	type AgentConfig,
	type AgentEvent,
...
import {
	...
	setClineDir,
	setHomeDir,
} from "@sctg/cline-shared/storage";
				...
				({
					run: vi.fn().mockImplementation(async () => {
						const snapshot = {
							...
							// see .backport-agent/customizations.yaml. Checkpoint
							// runCount is derived from genuine user messages in
							// the snapshot (checkpoint-run-counting.ts), so this
							// fake agent must mirror the real AgentRuntime's
							// behavior of seeding `state.messages` from
							// `initialMessages` instead of hardcoding `[]`.
							messages: [
								{
									id: "m1",
									role: "user" as const,
