...
describe("SessionRuntime.run — tracker wiring (P1 #3)", () => {
	it("aborts after maxConsecutiveMistakes failed-tool turns", async () => {
		...
		// .backport-agent/customizations.yaml. The mistake-limit stop notice
		// is a synthetic system-injected `role: "user"` message; it must be
		// tagged so checkpoint run counting doesn't mistake it for a
		// genuine user turn.
	...
	// FORK BUGFIX (keypool-live): checkpoint-runcount-fix - see
	// .backport-agent/customizations.yaml. When a consecutive-mistake limit
	// decision resolves to "continue" with guidance, the guidance is appended
	// as a synthetic system-injected `role: "user"` recovery notice - it must
	// be tagged so checkpoint run counting doesn't mistake it for a genuine
	// user turn.
	...
	// FORK BUGFIX (keypool-live): checkpoint-runcount-fix - see
	// .backport-agent/customizations.yaml. The soft-threshold loop warning is
	// a synthetic system-injected `role: "user"` message; it must be tagged
	// so checkpoint run counting (checkpoint-message-filter.ts) doesn't
	// mistake it for a genuine user turn.
