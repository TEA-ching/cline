...
`@sctg/cline-ui` has two opt-in layers:
...
without turning `@sctg/cline-ui` into a second agent runtime.

## Current status

`@sctg/cline-ui` is configured for public npm publication with its own version and
manual release workflow. Check availability with `npm view @sctg/cline-ui version`;
...
| --- | --- | --- | --- |
| Use only light/dark CSS variables | `@sctg/cline-ui/theme/tokens.css` | No | No |
| Use tokens through Tailwind utilities | `tokens.css` then `theme.css` | Tailwind v4 | No |
| Use the complete theme and shared base behavior | `@sctg/cline-ui/theme/index.css` | Tailwind v4 | No |
| Compose shared agent-chat presentation | `@sctg/cline-ui/components/agent-chat` plus its CSS | No, if tokens are mapped in plain CSS | React 18.3 or 19 |
...
There is no root JavaScript export and no `@sctg/cline-ui/theme` shorthand. Use the
...
@import "tailwindcss";
@import "@sctg/cline-ui/theme/index.css";
```
...
@import "@fontsource/azeret-mono/latin.css";
@import "tailwindcss";
@import "@sctg/cline-ui/theme/tokens.css";
@import "@sctg/cline-ui/theme/theme.css";
```

If the application later opts into shared base behavior, import
`@sctg/cline-ui/theme/base.css` after `theme.css`.
...
```css
@import "@sctg/cline-ui/theme/index.css";
@import "@sctg/cline-ui/components/agent-chat.css";
```
...
import {
  ...
  ToolActivityContent,
  ToolActivityTrigger,
} from "@sctg/cline-ui/components/agent-chat";
...
package depend on `@sctg/cline-core`, the Vercel AI SDK, desktop schemas, or transport
events.
...
```bash
bun -F @sctg/cline-ui storybook -- --host 0.0.0.0 --port 3490 --exact-port
```

Build the production Storybook bundle with:

```bash
bun -F @sctg/cline-ui build-storybook
```
...
Keep the following outside `@sctg/cline-ui`:
...
- [Desktop theme integration test (monorepo)](https://github.com/TEA-ching/cline/blob/main/apps/examples/desktop-app/webview/styles/theme-integration.test.ts)
