...
async function main(): Promise<void> {
	...
	try {
			...
			`${JSON.stringify(
				{
					...
					type: "module",
					dependencies: {
						"@sctg/cline-core": `file:${tarballs.core}`,
						"@sctg/cline-agents": `file:${tarballs.agents}`,
						"@sctg/cline-llms": `file:${tarballs.llms}`,
						"@sctg/cline-shared": `file:${tarballs.shared}`,
					},
				},
				null,
				2,
		...
		const smokeSource =
			nodeMajor >= 24
				? `
					const { SqliteSessionStore } = await import("@sctg/cline-core");
				...
				`
				: `
					const { resolveSessionBackend } = await import("@sctg/cline-core");
