...
const maxTokens = maxTokensArg
	? parseInt(maxTokensArg.split("=")[1])
	: Infinity;
...
@sctg/sdk is an advanced sdk for building AI-powered agents and tools. It provides a structured framework for orchestrating LLMs, managing tool execution, and handling complex workflows.
...
async function main() {
	...
	const patterns = ["packages/**/*.{ts,tsx,js,jsx,json}"];
	const ignore = [
		"package.json",
		"pnpm-lock.yaml",
		"yarn.lock",
	...
	if (verbose) {
		console.log(`\nToken breakdown:`);
		console.log(
			`  README:       ~${estimateTokens(readmeContent).toLocaleString()}`,
		);
		console.log(
			`  Architecture: ~${estimateTokens(ARCHITECTURE_PREAMBLE).toLocaleString()}`,
		);
		console.log(
			`  Source code:  ~${estimateTokens(codeFiles.map((f) => f.content).join("")).toLocaleString()}`,
		);
		console.log(
			`  Config:       ~${estimateTokens(configFiles.map((f) => f.content).join("")).toLocaleString()}`,
		);
	}
}
