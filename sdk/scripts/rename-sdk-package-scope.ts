...
function mapClineName(name: unknown): string {
	...
	// Avoid double "cline" in the name (e.g., @sctg/cline-hub-webview -> @sctg/cline-hub-webview)
	...
	try {
		...
		for (const pkgPath of packageJsonFiles) {
			try {
				...
				const serialized = replaceClineReferences(
		...
		// imports @sctg/cline-agents|llms|shared, which must become @sctg/cline-* at publish time.
		...
		const readmeFiles: string[] = [
			...
			...(fs.existsSync(appsCliDir)
				? findFilesByExtension(appsCliDir, "README.md")
				: []),
			...(fs.existsSync(appsClineHubDir)
				? findFilesByExtension(appsClineHubDir, "README.md")
				: []),
			...(fs.existsSync(appsExamplesDir)
				? findFilesByExtension(appsExamplesDir, "README.md")
				: []),
			...(fs.existsSync(chatbotDir)
				? findFilesByExtension(chatbotDir, "README.md")
				: []),
		];
		...
		if (repositoryUrl) {
			console.log(
				`Set repository.url to ${repositoryUrl} for trusted publishing provenance`,
			);
		}
