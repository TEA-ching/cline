...
describe("KeyPool", () => {
	...
	describe("resolveNextApiConfig", () => {
		it("should pick keys in round-robin fashion", () => {
			const res1 = resolveNextApiConfig(mockVault, "anthropic")
			res1?.apiKey.should.equal("key1")

			const res2 = resolveNextApiConfig(mockVault, "anthropic")
			res2?.apiKey.should.equal("key2")

			const res3 = resolveNextApiConfig(mockVault, "anthropic")
			res3?.apiKey.should.equal("key1")
		})

		it("should respect model selection", () => {
			const res = resolveNextApiConfig(mockVault, "anthropic", "claude-3-sonnet")
			res?.model.id.should.equal("claude-3-sonnet")
		})

		it("should skip unhealthy keys", () => {
			markKeyAsFailed("anthropic", "key1")
			markKeyAsFailed("anthropic", "key1")
			markKeyAsFailed("anthropic", "key1") // 3 failures = cooldown
		...
		it("should fallback to any key if all are unhealthy", () => {
			markKeyAsFailed("anthropic", "key1")
			markKeyAsFailed("anthropic", "key1")
			markKeyAsFailed("anthropic", "key1")
			markKeyAsFailed("anthropic", "key2")
			markKeyAsFailed("anthropic", "key2")
			markKeyAsFailed("anthropic", "key2")

			const res = resolveNextApiConfig(mockVault, "anthropic")
			should.exist(res)
			if (res) {
