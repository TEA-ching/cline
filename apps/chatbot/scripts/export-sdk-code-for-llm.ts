...
// CLI FLAGS PARSING
// =============================================================================
// This section handles command-line argument parsing to configure script behavior.
// Supported flags allow customization of output format, content, and token budget.
/**
 ...
 * - --no-index: Skip generating the companion llm.txt index file
 * - --max-tokens=N: Limit total token count to N (default: unlimited)
 * - --slim: Strip comments and blank lines to reduce token count
 * - --verbose: Show detailed token usage breakdown
 */
...
// outFile: Output markdown file path (default: "llm.md")
// slim: Boolean flag to enable slim mode (strips comments and blank lines)
// maxTokens: Maximum token budget for the output (default: Infinity)
// withIndex: Boolean flag to generate companion llm.txt index file
// verbose: Boolean flag to show detailed token usage breakdown
...
// HELPER FUNCTIONS
// =============================================================================
// Utility functions for processing files and content during export.
// These functions handle language detection, token estimation, and content
// transformation to prepare code for LLM consumption.
/**
...
// =============================================================================
// Predefined architecture overview that provides context about the SDK.
// This section contains static documentation that gets included in the generated output.
...
@sctg/cline-chatbot is a web-based AI chatbot application with a modular architecture designed for extensibility and secure API key management.

### Stack

- **Frontend**: React 19 + TypeScript + Vite
- **UI Framework**: HeroUI (v3) with Tailwind CSS
- **State Management**: React hooks + localStorage for persistence
- **Worker Architecture**: Web Worker for agent execution (isolated from UI)
- **Agent Runtime**: @sctg/cline-agents package for LLM interaction
- **Virtual File System**: In-memory VFS with file operations (vfs_*)
- **Web Tools**: Firecrawl integration for web search and scraping

### Key Patterns

- **Web Worker Isolation**: Agent runs in a separate worker thread to prevent UI blocking
- **Vault-based Auth**: Encrypted API key configuration fetched from remote vault
- **Key Pool Rotation**: Round-robin API key management with usage tracking
- **Tool System**: Built-in VFS tools + optional tools (deep research, news, SQL queries)
- **Session Persistence**: Chat sessions saved to localStorage with message history
- **Message Streaming**: Real-time token streaming with approval dialogs

### Directory Structure

- \`src/components/\` — React components (chat, auth, files, settings)
- \`src/hooks/\` — React hooks (useAgent, useVault, useKeypoolRotation)
- \`src/workers/\` — Web Worker for agent execution
- \`src/tools/\` — Tool definitions (VFS, Firecrawl, shell emulator)
- \`src/vfs/\` — Virtual file system implementation
- \`src/types/\` — TypeScript type definitions
- \`src/lib/\` — Utility libraries (crypto, keypool, model utils)
- \`src/session/\` — Session storage and export
`.trim();
...
async function main() {
    ...
    const ignore = [
        "package.json", "pnpm-lock.yaml", "yarn.lock", "lerna.json", "build/**", "scripts/**", "tests/**", "test/**", "bun.lockb", "tsconfig.json", "tsconfig.*.json", "vite.config.*.ts", "webpack.config.*.ts",
    ...
    md += "---\n";
    md += `title: "@sctg/cline-chatbot"\n`;
    md += `description: "Web-based AI chatbot with modular architecture, secure vault auth, and extensible tool system"\n`;
    md += `framework: react\n`;
    md += `stack: "React 19 + TypeScript + Vite + HeroUI + @sctg/cline-agents"\n`;
    md += `generated: "${now}"\n`;
    md += `slim_mode: ${slim}\n`;
    ...
    if (withIndex) {
        ...
        let idx = `@sctg/cline-chatbot — source index (${now})\n`;
        idx += `Stack: React 19 + TypeScript + Vite + HeroUI + @sctg/cline-agents\n\n`;
        idx += `FILES\n`;
