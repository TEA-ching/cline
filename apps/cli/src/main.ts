...
import type { ToolPolicy } from "@sctg/cline-core";

import { registerDisposable } from "@sctg/cline-shared";
...
async function createProviderSettingsManager() {
	const { ProviderSettingsManager } = await import("@sctg/cline-core");
...
export async function runCli(): Promise<void> {
	...
	const { setClineDir, setHomeDir } = await import(
	...
	const authCmd = program
		...
		.action(async (positionalProvider: string | undefined) => {
			...
			if (opts.config?.trim()) {
				const { setClineDir } = await import("@sctg/cline-shared/storage");
