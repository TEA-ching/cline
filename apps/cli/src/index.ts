...
import { disposeAll, initVcr, isHubDaemonProcess } from "@sctg/cline-shared";
// @ts-expect-error -- resolved by Bun's "file" import attribute at build time,
// not by the TS module resolver.
import parserWorkerPath from "@opentui/core/parser.worker" with {
	type: "file",
};
...
if (!isMainThread) {
	...
	void import("@sctg/cline-core/hub/daemon-entry");
} else {
