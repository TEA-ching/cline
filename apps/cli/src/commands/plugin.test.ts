...
describe("plugin install command", () => {
	...
	it("parses GitHub plugin file URLs as remote sources", () => {
		expect(
			parsePluginSource(
				"https://github.com/TEA-ching/cline/blob/main/sdk/examples/plugins/weather-metrics.ts",
			),
		).toEqual({
			type: "remote",
	...
	it("omits and removes host SDK packages from npm-sourced installs", async () => {
		...
		writeFileSync(
			...
			[
				...
				'mkdir -p "$prefix/node_modules/@sctg/cline-core"',
				...
				'printf \'%s\\n\' \'{"name":"@sctg/cline-core"}\' > "$prefix/node_modules/@sctg/cline-core/package.json"',
				"exit 0",
			].join("\n"),
			{ encoding: "utf8", mode: 0o755 },
		);
