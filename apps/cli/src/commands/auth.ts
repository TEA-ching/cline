...
import { getClineEnvironmentConfig } from "@sctg/cline-shared";
...
async function getCoreOAuthApi(): Promise<CoreOAuthApi> {
	if (!cachedCoreOAuthApi) {
		cachedCoreOAuthApi = import("@sctg/cline-core").then((module) => {
			...
			if (
				...
				throw new Error(
					"Installed @sctg/cline-core does not expose OAuth login helpers required by the CLI",
				);
			}
