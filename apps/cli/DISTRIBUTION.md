...
The Cline CLI (`cline`) is distributed as compiled binaries via npm. Users run `https://github.com/TEA-ching/cline/releases` and get a working `cline` command without needing Bun, Zig, or any other runtime installed.
...
| Package | Description |
|---|---|
| `@sctg/cline-cli-darwin-arm64` | macOS Apple Silicon binary |
| `@sctg/cline-cli-darwin-x64` | macOS Intel binary |
| `@sctg/cline-cli-linux-arm64` | Linux ARM binary |
| `@sctg/cline-cli-linux-x64` | Linux x64 binary |
| `@sctg/cline-cli-windows-x64` | Windows x64 binary |
| `@sctg/cline-cli-windows-arm64` | Windows ARM binary |
...
```json
{
  "name": "@sctg/cline-cli-darwin-arm64",
  "version": "0.1.0",
  "os": ["darwin"],
  "cpu": ["arm64"],
  "bin": {
...
```json
{
  ...
  "optionalDependencies": {
    "@sctg/cline-cli-darwin-arm64": "0.1.0",
    "@sctg/cline-cli-darwin-x64": "0.1.0",
    "@sctg/cline-cli-linux-arm64": "0.1.0",
    "@sctg/cline-cli-linux-x64": "0.1.0",
    "@sctg/cline-cli-windows-x64": "0.1.0",
    "@sctg/cline-cli-windows-arm64": "0.1.0"
  }
}
```
...
By default, `bun release cli` publishes with the npm dist-tag `latest` (what users get with `https://github.com/TEA-ching/cline/releases`). To publish under a different dist-tag like `next`, pass `--tag`:
...
```
User runs: https://github.com/TEA-ching/cline/releases
  |
  v
npm installs cline (wrapper package)
  + optionalDependencies (only the matching platform gets installed):
    - @sctg/cline-cli-darwin-arm64
    - @sctg/cline-cli-darwin-x64
    - @sctg/cline-cli-linux-arm64
    - @sctg/cline-cli-linux-x64
    - @sctg/cline-cli-windows-x64
    - @sctg/cline-cli-windows-arm64
  |
  v
...
2. Builds SDK packages (`bun run build:sdk`) and the CLI JS bundle (`bun -F @sctg/cline-cli build`)
...
2. Publishes all 6 platform packages in parallel (`@sctg/cline-cli-darwin-arm64`, etc.)
...
| `https://github.com/TEA-ching/cline/releases` | bin/cline resolver | Compiled binary | No |
