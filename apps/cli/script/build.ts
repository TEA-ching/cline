...
console.log(`Building @sctg/cline-cli v${version}`);
...
if (!buildOptions.skipSdkBuild) {
	...
	await $`bun -F @sctg/cline-cli build`.cwd(rootDir);
}
...
if (shouldBuildHubWebview()) {
	...
	await $`bun -F @sctg/cline-hub build:webview`.cwd(rootDir);
}
 ...
 */
function createReactDedupePlugin(
...
for (const item of targets) {
	...
	const name = `@sctg/cline-cli-${displayOs}-${item.arch}`;
