...
export async function getCapabilitiesSection(variant: PromptVariant, context: SystemPromptContext): Promise<string> {
	...
	const webToolsCapabilities =
		context.providerInfo.providerId !== "cline" || context.clineWebToolsEnabled === true
			? `\n- When the task requires or could benefit from getting up to date information on a topic (e.g. latest best practices, latest documentation, latest news, etc.), use the search_web tool to find current results, then use the fetch_web_content tool to retrieve and analyze the content from relevant URLs.`
			: ""
