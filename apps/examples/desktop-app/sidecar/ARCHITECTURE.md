# Sidecar Architecture — @sctg/cline-code

## Overview

The sidecar is a single Bun process that handles the desktop backend runtime directly.

It imports `@sctg/cline-core` directly and serves the Next.js frontend over HTTP + WebSocket.
...
```typescript
import { LocalRuntimeHost } from "@sctg/cline-core";
...
```typescript
import { ProviderSettingsManager, listLocalProviders, ... } from "@sctg/cline-core";
...
```typescript
import { SqliteSessionStore, resolveSessionBackend } from "@sctg/cline-core";
...
```typescript
import { ensureHubServer, sendHubCommand } from "@sctg/cline-core";
