...
```bash
npm add @sctg/cline-sdk
```

`@sctg/cline-sdk` re-exports everything from `@sctg/cline-core`. You only need `@sctg/cline-agents` or `@sctg/cline-llms` if you want lower-level control over the agent runtime or model gateway directly.
