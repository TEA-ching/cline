...
Dependencies under the `@cline/` scope are provided by the host runtime. The installer automatically strips these from the plugin's dependency list before running `npm install`, so declare any `@cline/*` package your plugin imports as an optional peer dependency. The host currently provides `@sctg/cline-sdk`, `@sctg/cline-core`, `@sctg/cline-agents`, `@sctg/cline-llms`, and `@sctg/cline-shared`.
