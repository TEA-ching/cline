...
| `fetch_web_content` | HTTP requests with HTML-to-markdown conversion|
| `search_web` | Web search|
| `ask_question` | Ask the user for input |

<Note>
`Agent` from `@sctg/cline-agents` does not include built-ins by default. You provide tools explicitly.
</Note>
