...
description: "API reference for the LLM provider gateway in @sctg/cline-llms."
---

`DefaultGateway` creates provider-backed `AgentModel` instances and exposes provider/model catalog helpers.

```typescript
import { DefaultGateway, createGateway } from "@sctg/cline-llms"
```
