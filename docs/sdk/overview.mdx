...
`@sctg/cline-sdk` exports all SDK packages: `@sctg/cline-core` for the full agent harness, `@sctg/cline-agents` for the stateless agent loop, `@sctg/cline-llms` for control over the model gateway, and `@sctg/cline-shared` for common utilities.
...
<Note>
  Here is a complete [quickstart example](https://github.com/TEA-ching/cline/tree/main/sdk/apps/examples/quickstart). Clone it and run `bun dev` to try it.
</Note>
...
| Package | Purpose |
|---------|---------|
| `@sctg/cline-sdk` | Public SDK surface (re-exports `@sctg/cline-core`) |
| `@sctg/cline-core` | Node runtime for sessions, built-in tools, persistence, hub support, automation |
| `@sctg/cline-agents` | Browser-compatible stateless agent execution loop |
| `@sctg/cline-llms` | Provider gateway and model catalogs |
| `@sctg/cline-shared` | Types, schemas, tool helpers, hooks, storage helpers |
