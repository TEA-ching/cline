...
Depends on: `@sctg/cline-shared`, `@sctg/cline-llms`, `@sctg/cline-agents`.
...
| `createTool` | Re-export from `@sctg/cline-shared` |

Methods on `AgentRuntime` include `run`, `continue`, `abort`, `subscribe`, `restore`, and `snapshot`.

Depends on: `@sctg/cline-shared`, `@sctg/cline-llms`.
...
`@sctg/cline-sdk` re-exports everything from `@sctg/cline-core`. Install `@sctg/cline-agents` or `@sctg/cline-llms` directly only if you need lower-level control.
...
`@sctg/cline-agents` exposes a browser-compatible runtime. It does not own session storage, built-in file/shell tools, hub transports, or Node-specific orchestration.

### Core as orchestration layer

`@sctg/cline-core` owns Node runtime integration: session persistence, built-in tools, automation, hub/remote transports, telemetry, and extension loading.
