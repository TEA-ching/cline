...
- `AgentRuntimeEvent` from `@sctg/cline-agents`, delivered through `agent.subscribe(listener)`.
- `AgentEvent` / `CoreSessionEvent` from `@sctg/cline-core`, delivered through core session adapters and `cline.subscribe(listener)`.
...
`AgentEvent` is the host-facing event shape used by `@sctg/cline-core` session orchestration. For direct `Agent` usage, prefer `agent.subscribe(...)` and `AgentRuntimeEvent`.
...
For a complete working example of streaming agent events to a browser via SSE, see the [multi-agent example](https://github.com/TEA-ching/cline/tree/main/sdk/apps/examples/multi-agent). It spawns multiple agents in parallel and streams each agent's events to separate UI cards.
