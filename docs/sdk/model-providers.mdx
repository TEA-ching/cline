...
The `@sctg/cline-llms` package provides provider registries, model catalogs, handlers, and a gateway. `@sctg/cline-agents` and `@sctg/cline-core` use this layer internally.
...
| KeypoolLive | `KEYPOOL_VAULT_URL`, `KEYPOOL_LIVE_SECRET`|
...
```typescript
import { DefaultGateway } from "@sctg/cline-llms"
