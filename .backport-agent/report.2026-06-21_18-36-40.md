...
**Run timestamp**: 2026-06-21T18:36:40.677Z
**Models used**: fast=`mistral/mistral-code-agent-latest`  powerful=`mistral/mistral-vibe-cli-latest`
**Prompt log**: `/Users/rlemeill/Development/tea-ching-cline/.backport-agent/run-1782064644799.prompts.jsonl`
...
**Date**: 2026-06-21T18:36:40.677Z
**Upstream ref**: `main`
**Fork ref**: `keypool-live`
**Sync branch**: `sync/upstream-main-2026-06-21-175833`
...
- `299a4a95` [low] fix(sdk): harden parallel tool guidance (#11598)
- `3c23f80a` [low] fix(sdk): deflake run_commands timeout telemetry tests
- `92f6e28f` [low] fix(cli): hide costs for Cline free models (#11686)

### ⚠️ Human review required

- `ee59f817` chore(cli): release v3.0.29
  - Conflicted files: apps/cli/CHANGELOG.md, apps/cli/package.json
- `4be36289` chore(cli): release v3.0.28
  - Conflicted files: apps/cli/CHANGELOG.md, apps/cli/package.json
- `cdff0846` chore(sdk): release v0.0.50
  - Conflicted files: apps/cli/CHANGELOG.md, apps/cli/package.json
- `2c4980f4` fix(sdk): resolve Cline Z.ai model metadata aliases (#11685)
  - Conflicted files: apps/cli/CHANGELOG.md, apps/cli/package.json
...
- Aborted 5 commits due to conflicts in CHANGELOG.md and package.json
- Validation failed due to build errors in the CLI
...
```mermaid
flowchart TD
    A["299a4a95 fix(sdk): harden parallel tool guidance (#11598)"] -->|low| B[applied ✅]
    C["3c23f80a fix(sdk): deflake run_commands timeout telemetry tests"] -->|low| D[applied ✅]
    E["92f6e28f fix(cli): hide costs for Cline free models (#11686)"] -->|low| F[applied ✅]
    G["ee59f817"] -->|needs-review| H[needs-review ⚠️]
    I["4be36289"] -->|needs-review| J[needs-review ⚠️]
    K["cdff0846"] -->|needs-review| L[needs-review ⚠️]
    M["2c4980f4"] -->|needs-review| N[needs-review ⚠️]
    O["27a78f02"] -->|needs-review| P[needs-review ⚠️]
    ...
    subgraph AI Tools
        Q[analyze_commit_for_backport] --> R[mistral/mistral-vibe-cli-latest]
        S[analyze_commit_for_backport] --> T[mistral/mistral-code-agent-latest]
        U[check_customization_compatibility] --> V[mistral/mistral-code-agent-latest]
    end
```
...
| Field | Value |
|---|---|
| **Timestamp** | 2026-06-21T17:58:03.202Z |
...
index ac81bc40d..3daaf5ed1 100644
--- a/bun.lock
+++ b/bun.lock
@@ -372,7 +372,7 @@
     },
...
+    "@opentelemetry/sdk-trace-base": ["@opentelemetry/sdk-trace-base@2.8.0", "", { "dependencies": { "@opentelemetry/core": "2.8.0", "@opentelemetry/resources": "2.8.0", "@opentelemetry/semantic-conventions": "^1.29.0" }, "peerDependencies": { "@opentelemetry/api": ">=1.3.0 <1.10.0" } }, "sha512-mhU4jp+vW0mGbFRd+GeXHvmfA4aD
... [truncated]
...
| Field | Value |
|---|---|
| **Timestamp** | 2026-06-21T17:58:07.051Z |
| **Tool** | `analyze_commit_for_backport` |
...
index 9fef86d5f..d76096323 100644
...
index eb05567fd..5302bd6d7 100644
...
index 2b7df16cf..662a4d4cf 100644
...
index 702437050..bcb6dfedb 100644
...
index f919a0f8d..40a1a3dfc 100644
...
index d5b55ee58..84c14120e 100644
...
index e45849277..0c3ef62f6 100644
...
index 865bc9dcd..844c60c2c 100644
...
```json
{
  "summary": "Fixes Cline Z.ai model metadata aliases by preferring Vercel-style IDs over OpenRouter aliases. Adds model ID alias resolution logic and updates provider defaults.",
  "keyChanges": [
    "Adds model ID alias handling for Cline provider",
    "Prefers 'zai/' over 'z-ai/' model IDs in Cline",
    "Updates provider-defaults to use canonical model IDs",
    "Adds new model-id-aliases module with tests",
    "Modifies builtins.ts to use Cline-specific model factory"
  ],
  "backportComplexity": "moderate",
  "semanticRiskFactors": [
    "changes provider model resolution logic",
    "introduces new model ID alias system"
  ],
...
| Field | Value |
|---|---|
| **Timestamp** | 2026-06-21T17:58:11.175Z |
| **Tool** | `analyze_commit_for_backport` |
| **Model** | `mistral/mistral-vibe-cli-latest` |
| **Duration** | 4035 ms |
| **Tokens in** | 18386 |
...
diff --git a/bun.lock b/bun.lock
index 3daaf5ed1..19d7bfefb 100644
--- a/bun.lock
+++ b/bun.lock
@@ -19,7 +19,7 @@
     },
...
```
{
  "summary": "SDK version bump to v0.0.51 with dependency updates and lockfile regeneration.",
  "keyChanges": [
    "Bumps SDK packages to v0.0.51",
    "Updates @anthropic-ai/claude-agent-sdk to 0.3.177",
    "Updates ai and @ai-sdk packages",
    "Regenerates bun.lock",
    "Updates catalog.generated.ts"
  ],
  "backportComplexity": "moderate",
  "semanticRiskFactors": [
    "Major version bumps in Anthropic SDK (0.2.x → 0.3.x)",
    "New dependencies added (standardwebhooks, @stablelib/base64)"
  ],
...
| Field | Value |
|---|---|
| **Timestamp** | 2026-06-21T17:58:15.093Z |
...
Declared fork customisations:
  1. Pattern: sdk/packages/llms/src/providers/builtins.ts
     Description: Custom Cline provider integration for Keypool Live
...
{
  "compatible": true,
  "affectedCustomizations": ["Custom Cline provider integration for Keypool Live"],
  "semanticConflicts": [],
  "warnings": ["The upstream change modifies Cline model metadata handling, which could affect Keypool Live if it relies on specific model IDs."],
  "recommendation": "Review the changes to ensure Keypool Live's custom Cline provider integration remains compatible with the new model ID alias handling."
}
```
````
...
**Total AI time**: 15127 ms across 4 call(s)
**Total input tokens**: 49276
**Total output tokens**: 529

| Tool | Calls | Total ms | Avg ms | Input tok | Output tok |
|---|---|---|---|---|---|
| `analyze_commit_for_backport` | 3 | 13263 | 4421 | 43922 | 423 |
| `check_customization_compatibility` | 1 | 1864 | 1864 | 5354 | 106 |
...
|---|---|---|---|
| 17:58:03 | `analyze_commit_for_backport` | `analysis_complete` | sha="cdff084652e46a4f9aa7e8f4ed57766a3e10f71, recommendation="apply", complexity="trivial" |
| 17:58:07 | `analyze_commit_for_backport` | `analysis_complete` | sha="2c4980f42a73968f426327036a00aeae3c42044, recommendation="apply-with-care", complexity="moderate" |
| 17:58:11 | `analyze_commit_for_backport` | `analysis_complete` | sha="27a78f024834f30f9c8f8f8ff08c76aa2a505e9, recommendation="review-required", complexity="moderate" |
| 17:58:15 | `check_customization_compatibility` | `compatibility_check_complete` | compatible=true, affectedCustomizations=["Custom Cline provider integration for , semanticConflictsCount=0 |
| 17:58:29 | `reconcile_ai_assessments` | `no_contradiction` | sha="2c4980f42a73968f426327036a00aeae3c42044, analyzeRecommendation="apply-with-care", compatibilityCompatible=true |
